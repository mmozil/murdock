...
- **Fonte/cor = Harvey.ai real (inspecionado via browser + comparação visual)**: fundo `#0f0e0d` (= `--ink`, idêntico ao Harvey) + texto ivory/branco. Harvey usa fontes **proprietárias** (`HarveySerifFont` custom + `HarveySansDiatypeVariable` = **Diatype/Dinamo**, pagas, fora do Google Fonts → NÃO embarcáveis). Free mais próximas (escolhidas comparando renders lado a lado contra a HarveySerif real): **Fraunces** (`--font-serif`, variável — peso/contraste/serifas afiadas casados com a Harvey; testadas e descartadas: Newsreader=pontas arredondadas, Playfair=contraste alto demais, Libre Caslon/Prata=leves demais por serem single-weight 400) + **Inter** (`--font-sans`, ≈ Diatype). Harvey é **monocromática, sem accent cromático**: o dourado `#c9a96e` (resíduo Murdock) foi REMOVIDO — `--accent` = `#fafaf9` (ivory); botões primários = ivory bg + ink text (igual "Request a Demo"). `--green`/`--red` só como feedback semântico.
