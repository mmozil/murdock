...
- **Fonte/cor = Harvey.ai real (inspecionado via browser)**: fundo `#0f0e0d` (= `--ink`, idêntico ao Harvey) + texto ivory/branco. Harvey usa fontes **proprietárias** (`HarveySerifFont` + Diatype/Dinamo) → usadas as free mais próximas: **Newsreader** (serif, `--font-serif`) + **Inter** (sans, `--font-sans`). Harvey é **monocromática, sem accent cromático**: o dourado `#c9a96e` original (resíduo Murdock) foi REMOVIDO — `--accent` agora = `#fafaf9` (ivory); botões primários = ivory bg + ink text (igual "Request a Demo" do harvey.ai). `--green`/`--red` ficam só como feedback semântico.
